###################################################################
###  Generate Empirical Pvalue - EDGER
###################################################################
...
load(cfg$diff_result_rdata)
# load("/spstorage/USERS/sung/projects/KMAP_Signature/rdata/allplate/emp_deseq_res_list_withoutzerocount.RData")
...
calculate_mode <- cfg$calculate_mode
...
edger_pval_list <- lapply(1:length(DMSO_count_filtered_list), function(xx){
	tcheck <- proc.time()
	print(paste0(xx, "th plate processing"))

	DMSO_count <- DMSO_count_filtered_list[[xx]]
	pval_list <- edger_DMSO(DMSO_count, w)
	names(pval_list) <- rownames(DMSO_count)
	
	print((proc.time() - tcheck)/60)
	return(pval_list)
})
names(edger_pval_list) <- platenum

emppval_from_edger_perbin_list <- DMSO_pval_to_empiricalpval(edger_pval_list)
names(emppval_from_edger_perbin_list) <- platenum

loess_edger_list <- loess_empirical(emppval_from_edger_perbin_list)
names(loess_edger_list) <- platenum

edgerres_list <- mclapply(1:length(drug_count_list), mc.cores = core, function(xx){
	...
	deseqres <- edger_drug(drug_count, version, dose)
	return(deseqres)
	})
names(edgerres_list) <- names(drug_count_list)

edger_group_quantile_list <- group_quantile(edger_pval_list)
names(edger_group_quantile_list) <- platenum

emp_edger_res_list <- predict_empiricalPvalue(edgerres_list, loess_edger_list, edger_pval_list, edger_group_quantile_list)
names(emp_edger_res_list) <- names(edgerres_list_partial)
